...
#define NullCode (-1)
		...
		int available, bits, code, clear, code_mask, code_size, count, end_of_information, in_code, offset, old_code, pass, y;
...
bool ImageLib::WritePNGImage(const std::string& theFileName, Image* theImage)
{
	...
	png_set_IHDR(png_ptr, info_ptr, theImage->mWidth, theImage->mHeight, 8, PNG_COLOR_TYPE_RGB_ALPHA, PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_BASE,
...
void jpeg_pak_src(j_decompress_ptr cinfo, PFILE* infile)
{
	...
	{ /* first time for this JPEG object? */
		cinfo->src = (struct jpeg_source_mgr*)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_PERMANENT, sizeof(pak_source_mgr));
	...
	if (gJ2KCodec != NULL)
	{
		...
		if (loadFuncs)
		{
			...
			*((void**)&fJ2K_getErrorStr) = (void*)::GetProcAddress(gJ2KCodec, "_J2K_getErrorStr@4");
			...
			if (!(fJ2K_getVersion != NULL && fJ2K_Unlock != NULL && fJ2K_OpenCustom != NULL && fJ2K_OpenFile != NULL && fJ2K_Close != NULL &&
					fJ2K_GetInfo != NULL && fJ2K_GetResolutionDimensions != NULL && fJ2K_Decode != NULL && fJ2K_getLastError != NULL &&
