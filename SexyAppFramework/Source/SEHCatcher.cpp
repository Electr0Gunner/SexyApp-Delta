...
void SEHCatcher::DoHandleDebugEvent(LPEXCEPTION_POINTERS lpEP)
{
	...
	sprintf(aBuffer, ("RAX:%016llX RBX:%016llX RCX:%016llX RDX:%016llX RSI:%016llX RDI:%016llX\r\n"), lpEP->ContextRecord->Rax, lpEP->ContextRecord->Rbx,
		lpEP->ContextRecord->Rcx, lpEP->ContextRecord->Rdx, lpEP->ContextRecord->Rsi, lpEP->ContextRecord->Rdi);
	aDebugDump += aBuffer;
	...
	aDebugDump += aBuffer;
#else
#error Unsupported architecture in SEHCatcher::DoHandleDebugEvent
#endif
		...
		if (wParam == 0)
		{
			...
			if ((aResult != HTTPTransfer::RESULT_NOT_COMPLETED) && (aResult != HTTPTransfer::RESULT_NOT_STARTED))
			{
				if ((aResult != HTTPTransfer::RESULT_DONE) && (aResult != HTTPTransfer::RESULT_ABORTED))
				{
					...
					if (::MessageBoxW(hWnd, SEHCatcher::mSubmitErrorMessage.c_str(), L"Connection Failed", MB_RETRYCANCEL | MB_APPLMODAL) == IDRETRY)
					{
