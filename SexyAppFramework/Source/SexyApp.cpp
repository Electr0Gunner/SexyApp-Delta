...
void SexyApp::Init()
{
	SEHCatcher::mCrashMessage =
		...
		"us resolve this issue in the future.rnrn"
	...
	OutputDebugString(StrFormat("BuildNum: %drn", mBuildNum).c_str());
	OutputDebugString(StrFormat("BuildDate: %srn", mBuildDate.c_str()).c_str());
	SexyAppBase::Init();
	if (IsScreenSaver())
		mSkipAd = true;
	mTimesExecuted++;
