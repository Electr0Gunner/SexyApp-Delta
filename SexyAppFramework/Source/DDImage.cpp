...
void DDImage::NormalBlt(Image* theImage, int theX, int theY, const Rect& theSrcRect, const Color& theColor)
{
	...
	if (aMemoryImage != NULL)
	{
		...
		// TODO:
		if ((aMemoryImage->mIsVolatile) && ((aDDImage == NULL) || (aDDImage->mSurface == NULL)) && (!mNoLock) && (theColor == Color::White))
		{
...
void DDImage::Blt(Image* theImage, int theX, int theY, const Rect& theSrcRect, const Color& theColor, int theDrawMode)
{
	...
	if (Check3D(this))
	{
		...
		if ((aDDImage != NULL) && (aDDImage->mSurface != NULL) && (mDDInterface->mD3DInterface->mTransformStack.empty()) &&
	...
	Image* theImage, const Rect& theDestRectOrig, const Rect& theSrcRectOrig, const Rect& theClipRect, const Color& theColor, int theDrawMode, bool fastStretch)
{
	...
	if (fastStretch)
	{
		if ((aSrcDDImage != NULL) && (theColor == Color::White) && (theDrawMode == Graphics::DRAWMODE_NORMAL) && (!aSrcDDImage->mHasAlpha) &&
			(aSrcDDImage->GetSurface() != NULL))
		{
