...
bool DSoundManager::LoadWAVSound(unsigned int theSfxID, const std::string& theFilename)
{
	...
	while (!p_feof(fp))
	{
		...
		else if (strcmp(aChunkType, "dep ") == 0)
		{
			...
			if ((aSavedFileTime.dwHighDateTime != anActualFileTime.dwHighDateTime) || (aSavedFileTime.dwLowDateTime != anActualFileTime.dwLowDateTime))
				return false;
		}
...
{
	ov_callbacks callbacks = { (size_t (*)(void*, size_t, size_t, void*))p_fread, (int (*)(void*, ogg_int64_t, int))p_fseek64_wrap, (int (*)(void*))p_fclose,
		(long (*)(void*))p_ftell };
