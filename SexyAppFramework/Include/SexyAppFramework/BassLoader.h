...
namespace Sexy
{
	struct BASS_INSTANCE
	{
		...
		BOOL BASS_ChannelSetFlags(DWORD handle, DWORD flags);
		BOOL BASS_ChannelSlideAttributes(DWORD handle, int freq, int volume, int pan, DWORD time);
		DWORD BASS_MusicGetAttribute(DWORD handle, DWORD attrib);
		void BASS_MusicSetAttribute(DWORD handle, DWORD attrib, DWORD value);
		...
		BOOL(WINAPI* BASS_Init)(int device, DWORD freq, DWORD flags, HWND win, const void* dsguid);
		void(WINAPI* BASS_Free)();
		BOOL(WINAPI* BASS_Stop)();
		BOOL(WINAPI* BASS_Start)();
		BOOL(WINAPI* BASS_SetVolume)(float volume);
		...
		HSYNC(WINAPI* BASS_ChannelSetSync)(DWORD handle, DWORD theType, QWORD theParam, SYNCPROC* proc, void* user);
