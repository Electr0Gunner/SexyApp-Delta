...
HRESULT DDInterface::CreateSurface(DDSURFACEDESC2* theDesc, LPDIRECTDRAWSURFACE* theSurface, void*)
{
	...
	gOptimizeSoftwareDrawing =
		aDesc.ddpfPixelFormat.dwBBitMask < aDesc.ddpfPixelFormat.dwGBitMask && aDesc.ddpfPixelFormat.dwGBitMask < aDesc.ddpfPixelFormat.dwRBitMask;
#endif
...
bool DDInterface::CopyBitmap(LPDIRECTDRAWSURFACE theSurface, HBITMAP theBitmap, int theX, int theY, int theWidth, int theHeight)
{
	...
	if (theBitmap == nullptr || theSurface == nullptr)
		return false;
	...
	if (hdcImage != nullptr)
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			{
				StretchBlt(hdc, 0, 0, aDesc.dwWidth, aDesc.dwHeight, hdcImage, theX, theY, theWidth, theHeight, SRCCOPY);
			}
...
void DDInterface::RestoreOldCursorAreaFrom(LPDIRECTDRAWSURFACE theSurface, bool adjust)
{
	...
	{
		Rect aSexyScreenRect(mCursorX - (mCursorWidth / 2), mCursorY - (mCursorHeight / 2), mCursorWidth, mCursorHeight);

		Rect aClippedScreenRect = aSexyScreenRect.Intersection(Rect(0, 0, mWidth, mHeight));

		Rect aSexyLocalRect(
			aClippedScreenRect.mX - aSexyScreenRect.mX, aClippedScreenRect.mY - aSexyScreenRect.mY, aClippedScreenRect.mWidth, aClippedScreenRect.mHeight);
...
void DDInterface::DrawCursorTo(LPDIRECTDRAWSURFACE theSurface, bool adjust)
{
	if ((mCursorImage != nullptr) && (mPrimarySurface != nullptr))
	{
		...
		else
		{
			...
			aNewCursorAreaG.DrawImage(mCursorImage, (mCursorWidth - mCursorImage->mWidth) / 2, (mCursorHeight - mCursorImage->mHeight) / 2);
...
void DDInterface::MoveCursorTo(LPDIRECTDRAWSURFACE theSurface, bool adjust, int theNewCursorX, int theNewCursorY)
{
	...
	if ((mCursorImage != nullptr) && (mPrimarySurface != nullptr))
	{
		...
		// NEW RECTANGLES
		Rect aNewSexyScreenRect(theNewCursorX - (mCursorWidth / 2), theNewCursorY - (mCursorHeight / 2), mCursorWidth, mCursorHeight);

		Rect aNewClippedScreenRect = aNewSexyScreenRect.Intersection(mPresentationRect);
		Rect aNewSexyLocalRect(aNewClippedScreenRect.mX - aNewSexyScreenRect.mX, aNewClippedScreenRect.mY - aNewSexyScreenRect.mY, aNewClippedScreenRect.mWidth,
		...
		{
			Rect aSexyDestAreaRect(aSexySrcAreaRect.mX - aSexyOrigSrcAreaRect.mX, aSexySrcAreaRect.mY - aSexyOrigSrcAreaRect.mY, aSexySrcAreaRect.mWidth,
			...
			aOldCursorAreaG.DrawImage(mCursorImage, theNewCursorX - mCursorX + (mCursorWidth - mCursorImage->mWidth) / 2,
