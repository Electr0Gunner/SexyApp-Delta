		...
		{ // draw horizontal segments
			...
			if ((xl <= xr) && (aSpanPos < MAX_TEMP_SPANS))
			{
				...
				else
				{
					...
					{
						double m = 1.0 / mPFActiveEdgeList[j].mDX, b = mPFActiveEdgeList[j].b, c = fabs(mPFActiveEdgeList[j].mDX);
						do
						{
							double _y = m * xl + b;
					...
					{
						double m = 1.0 / mPFActiveEdgeList[j + 1].mDX, b = mPFActiveEdgeList[j + 1].b, c = fabs(mPFActiveEdgeList[j + 1].mDX);
						do
						{
							double _y = m * xr + b;
...
void Graphics::DrawImage(Image* theImage, int theX, int theY, const Rect& theSrcRect)
{
	...
	if (mScaleX != 1 || mScaleY != 1)
	{
		Rect aDestRect(mScaleOrigX + floor((theX - mScaleOrigX) * mScaleX), mScaleOrigY + floor((theY - mScaleOrigY) * mScaleY),
...
void Graphics::DrawImageRotatedF(Image* theImage, float theX, float theY, double theRot, float theRotCenterX, float theRotCenterY, const Rect* theSrcRect)
{
	...
	if (theSrcRect == NULL)
	{
		...
		mDestImage->BltRotated(
			theImage, theX, theY, aSrcRect, mClipRect, mColorizeImages ? mColor : Color::White, mDrawMode, theRot, theRotCenterX, theRotCenterY);
	}
	else
		mDestImage->BltRotated(
			theImage, theX, theY, *theSrcRect, mClipRect, mColorizeImages ? mColor : Color::White, mDrawMode, theRot, theRotCenterX, theRotCenterY);
}
...
void Graphics::DrawImageMatrix(Image* theImage, const SexyMatrix3& theMatrix, const Rect& theSrcRect, float x, float y)
{
	mDestImage->BltMatrix(
		theImage, x + mTransX, y + mTransY, theMatrix, mClipRect, mColorizeImages ? mColor : Color::White, mDrawMode, theSrcRect, mLinearBlend);
}
...
void Graphics::DrawTrianglesTex(Image* theTexture, const TriVertex theVertices[][3], int theNumTriangles)
{
	mDestImage->BltTrianglesTex(
		theTexture, theVertices, theNumTriangles, mClipRect, mColorizeImages ? mColor : Color::White, mDrawMode, mTransX, mTransY, mLinearBlend);
}
...
{
	mClipRect = Rect(0, 0, mDestImage->GetWidth(), mDestImage->GetHeight()).Intersection(Rect(theX + mTransX, theY + mTransY, theWidth, theHeight));
}
...
int Graphics::WriteString(
	const SexyString& theString, int theX, int theY, int theWidth, int theJustification, bool drawString, int theOffset, int theLength, int theOldColor)
{
	Font* aFont = GetFont();
	if (theOldColor == -1)
...
static int WriteWordWrappedHelper(Graphics* g, const SexyString& theString, int theX, int theY, int theWidth, int theJustification, bool drawString,
...
int Graphics::WriteWordWrapped(
	const Rect& theRect, const SexyString& theLine, int theLineSpacing, int theJustification, int* theMaxWidth, int theMaxChars, int* theLastWidth)
{
	...
	while (aCurPos < theLine.length())
	{
		...
		if (aCurWidth > theRect.mWidth) // need to wrap
		{
			...
			if (aSpacePos != -1)
			{
				...
				{
					WriteWordWrappedHelper(this, theLine, theRect.mX + anIndentX, theRect.mY + aYOffset, theRect.mWidth, theJustification, true, aLineStartPos,
			...
			else
			{
				...
				aWrittenWidth = WriteWordWrappedHelper(this, theLine, theRect.mX + anIndentX, theRect.mY + aYOffset, theRect.mWidth, theJustification, true,
					aLineStartPos, aCurPos - aLineStartPos, anOrigColorInt, theMaxChars);
	...
	{
		int aWrittenWidth = WriteWordWrappedHelper(this, theLine, theRect.mX + anIndentX, theRect.mY + aYOffset, theRect.mWidth, theJustification, true,
