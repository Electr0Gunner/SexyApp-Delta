...
void SWHelper::SWDrawShape(XYZStruct* theVerts, int theNumVerts, MemoryImage* theImage, const Color& theColor, int theDrawMode, const Rect& theClipRect,
		...
		if (vCount)
		{
			...
			if (vCount > 3)
			{
				for (unsigned int extraVert = 2; extraVert < vCount - 1; ++extraVert)
				{
					...
					SWDrawTriangle(
						textured, talpha, vertexColor, globalargb, pVerts, pFrameBuffer, thePitch, &textureInfo, globalDiffuse, thePixelFormat, blend);
				}
			}
		}
	...
	const unsigned int bytepitch, const SWTextureInfo* textureInfo, SWDiffuse& globalDiffuse, int thePixelFormat, bool blend)
{
	...
	//	#include "SWTri/SWTri_DrawTriangleInc2.cpp"
}
