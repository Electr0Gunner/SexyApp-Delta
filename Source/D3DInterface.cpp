...
{
	if (theFormat->dwFlags == (DDPF_ALPHAPIXELS | DDPF_RGB) && theFormat->dwRGBBitCount == 32 && theFormat->dwRGBAlphaBitMask == 0xFF000000 &&
		theFormat->dwRBitMask == 0x00FF0000 && theFormat->dwGBitMask == 0x0000FF00 && theFormat->dwBBitMask == 0x000000FF)
	{
		return PixelFormat_A8R8G8B8;
	}

	if (theFormat->dwFlags == (DDPF_ALPHAPIXELS | DDPF_RGB) && theFormat->dwRGBBitCount == 16 && theFormat->dwRGBAlphaBitMask == 0xF000 &&
		theFormat->dwRBitMask == 0x0F00 && theFormat->dwGBitMask == 0x00F0 && theFormat->dwBBitMask == 0x000F)
	{
		return PixelFormat_A4R4G4B4;
	}

	if (theFormat->dwFlags == DDPF_RGB && theFormat->dwRGBBitCount == 16 && theFormat->dwRGBAlphaBitMask == 0x0000 && theFormat->dwRBitMask == 0xF800 &&
		theFormat->dwGBitMask == 0x07E0 && theFormat->dwBBitMask == 0x001F)
	{
...
bool D3DInterface::PreDraw()
{
	...
	if (!mSceneBegun)
	{
		...
		if (!SUCCEEDED(mD3DDevice->SetRenderTarget(
				mDDSDrawSurface, 0))) // this happens when there's been a mode switch (this caused the nvidia screensaver bluescreen)
		{
...
static void CopyTexturePalette8ToImage(
	void* theDest, DWORD theDestPitch, MemoryImage* theImage, int offx, int offy, int theWidth, int theHeight, LPDIRECTDRAWPALETTE thePalette)
{
	char* srcRow = (char*)theDest;
	DWORD* dstRow = theImage->GetBits() + offy * theImage->GetWidth() + offx;
...
{
	if (mPixelFormat == PixelFormat_Unknown || theImage->mWidth != mWidth || theImage->mHeight != mHeight || theImage->mBitsChangedCount != mBitsChangedCount ||
...
void TextureData::Blt(LPDIRECT3DDEVICE7 theDevice, float theX, float theY, const Rect& theSrcRect, const Color& theColor)
{
	...
	while (srcY < srcBottom)
	{
		...
		while (srcX < srcRight)
		{
			...
			D3DTLVERTEX aVertex[4] = { { x, y, 0, 1, aColor, 0, u1, v1 }, { x, y + aHeight, 0, 1, aColor, 0, u1, v2 },
				{ x + aWidth, y, 0, 1, aColor, 0, u2, v1 }, { x + aWidth, y + aHeight, 0, 1, aColor, 0, u2, v2 } };
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
template <class Pred> void PointClipper<Pred>::ClipPoint(int n, float clipVal, const D3DTLVERTEX& v1, const D3DTLVERTEX& v2, VertexList& out)
{
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
void TextureData::BltTransformed(LPDIRECT3DDEVICE7 theDevice, const SexyMatrix3& theTrans, const Rect& theSrcRect, const Color& theColor,
	const Rect* theClipRect, float theX, float theY, bool center)
{
	...
	while (srcY < srcBottom)
	{
		...
		while (srcX < srcRight)
		{
			...
			D3DTLVERTEX aVertex[4] = { { tp[0].x, tp[0].y, 0, 1, aColor, 0, u1, v1 }, { tp[1].x, tp[1].y, 0, 1, aColor, 0, u1, v2 },
				{ tp[2].x, tp[2].y, 0, 1, aColor, 0, u2, v1 }, { tp[3].x, tp[3].y, 0, 1, aColor, 0, u2, v2 } };
...
void TextureData::BltTriangles(LPDIRECT3DDEVICE7 theDevice, const TriVertex theVertices[][3], int theNumTriangles, DWORD theColor, float tx, float ty)
{
		...
		for (int aTriangleNum = 0; aTriangleNum < theNumTriangles; aTriangleNum++)
		{
			...
			if ((aVertexCacheNum == 100) || (aTriangleNum == theNumTriangles - 1))
			{
				...
				D3DInterface::CheckDXError(
					theDevice->DrawPrimitive(D3DPT_TRIANGLELIST, gVertexType, aVertexCache, aVertexCacheNum * 3, 0), "DrawPrimitive (TriList)");
				aVertexCacheNum = 0;
			}
		}
	}
	else
	{
		for (int aTriangleNum = 0; aTriangleNum < theNumTriangles; aTriangleNum++)
		{
			...
			D3DTLVERTEX aVertex[3] = { { aTriVerts[0].x + tx, aTriVerts[0].y + ty, 0, 1, GetColorFromTriVertex(aTriVerts[0], theColor), 0,
										   aTriVerts[0].u * mMaxTotalU, aTriVerts[0].v * mMaxTotalV },
				{ aTriVerts[1].x + tx, aTriVerts[1].y + ty, 0, 1, GetColorFromTriVertex(aTriVerts[1], theColor), 0, aTriVerts[1].u * mMaxTotalU,
					aTriVerts[1].v * mMaxTotalV },
				{ aTriVerts[2].x + tx, aTriVerts[2].y + ty, 0, 1, GetColorFromTriVertex(aTriVerts[2], theColor), 0, aTriVerts[2].u * mMaxTotalU,
			...
			for (i = aTop; i < aBottom; i++)
			{
				for (j = aLeft; j < aRight; j++)
				{
					...
					if (aList.size() >= 3)
					{
						...
						D3DInterface::CheckDXError(
							theDevice->DrawPrimitive(D3DPT_TRIANGLEFAN, gVertexType, &aList[0], aList.size(), 0), "DrawPrimitive (Tri) 4");
						...
						/*				CheckDXError(theDevice->SetTexture(0, NULL),"SetTexture NULL");
										...
										CheckDXError(theDevice->DrawPrimitive(D3DPT_TRIANGLEFAN, gVertexType, &aList[0], aList.size(),
						   D3DDP_WAIT),"DrawPrimitive (Tri)"); theDevice->SetRenderState(D3DRENDERSTATE_FILLMODE, D3DFILL_SOLID);*/
					}
				}
			}
		}
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
void D3DInterface::StretchBlt(Image* theImage, const Rect& theDestRect, const Rect& theSrcRect, const Rect* theClipRect, const Color& theColor, int theDrawMode,
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
void D3DInterface::BltRotated(Image* theImage, float theX, float theY, const Rect* theClipRect, const Color& theColor, int theDrawMode, double theRot,
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
void D3DInterface::BltTransformed(Image* theImage, const Rect* theClipRect, const Color& theColor, int theDrawMode, const Rect& theSrcRect,
...
void D3DInterface::DrawLine(double theStartX, double theStartY, double theEndX, double theEndY, const Color& theColor, int theDrawMode)
{
	...
	D3DTLVERTEX aVertex[3] = { { x1, y1, 0, 1, aColor, 0, 0, 0 }, { x2, y2, 0, 1, aColor, 0, 0, 0 }, { x2 + 0.5f, y2 + 0.5f, 0, 1, aColor, 0, 0, 0 } };
...
void D3DInterface::FillRect(const Rect& theRect, const Color& theColor, int theDrawMode)
{
	...
	D3DTLVERTEX aVertex[4] = { { x, y, 0, 1, aColor, 0, 0, 0 }, { x, y + aHeight, 0, 1, aColor, 0, 0, 0 }, { x + aWidth, y, 0, 1, aColor, 0, 0, 0 },
...
void D3DInterface::DrawTriangle(const TriVertex& p1, const TriVertex& p2, const TriVertex& p3, const Color& theColor, int theDrawMode)
{
	...
	D3DTLVERTEX aVertex[3] = { { p1.x, p1.y, 0, 1, GetColorFromTriVertex(p1, aColor), 0, 0, 0 },
		{ p2.x, p2.y, 0, 1, GetColorFromTriVertex(p2, aColor), 0, 0, 0 }, { p3.x, p3.y, 0, 1, GetColorFromTriVertex(p3, aColor), 0, 0, 0 } };
...
void D3DInterface::DrawTriangleTex(
	const TriVertex& p1, const TriVertex& p2, const TriVertex& p3, const Color& theColor, int theDrawMode, Image* theTexture, bool blend)
{
...
void D3DInterface::DrawTrianglesTex(
	const TriVertex theVertices[][3], int theNumTriangles, const Color& theColor, int theDrawMode, Image* theTexture, float tx, float ty, bool blend)
{
	if (!PreDraw())
		return;
...
void D3DInterface::DrawTrianglesTexStrip(
	const TriVertex theVertices[], int theNumTriangles, const Color& theColor, int theDrawMode, Image* theTexture, float tx, float ty, bool blend)
{
